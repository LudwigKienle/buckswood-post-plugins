...
float FakeDiagnosticCore::clamp01(float value)
{
    return std::min(1.0f, std::max(0.0f, value));
}

float FakeDiagnosticCore::luma(Pixel pixel)
{
    return pixel.r * 0.2126f + pixel.g * 0.7152f + pixel.b * 0.0722f;
}

float FakeDiagnosticCore::max3(float a, float b, float c)
{
    return std::max(a, std::max(b, c));
}

float FakeDiagnosticCore::min3(float a, float b, float c)
{
    return std::min(a, std::min(b, c));
}

float FakeDiagnosticCore::smoothstep(float edge0, float edge1, float x)
{
    if (edge0 == edge1) {
...
float FakeDiagnosticCore::hash01(int x, int y, int z)
{
...
Pixel FakeDiagnosticCore::add(Pixel a, Pixel b)
{
    return Pixel{a.r + b.r, a.g + b.g, a.b + b.b, a.a + b.a};
}

Pixel FakeDiagnosticCore::sub(Pixel a, Pixel b)
{
    return Pixel{a.r - b.r, a.g - b.g, a.b - b.b, a.a - b.a};
}

Pixel FakeDiagnosticCore::mul(Pixel a, float scalar)
{
    return Pixel{a.r * scalar, a.g * scalar, a.b * scalar, a.a * scalar};
}

Pixel FakeDiagnosticCore::mix(Pixel a, Pixel b, float t)
{
    t = clamp01(t);
    return Pixel{
        a.r + (b.r - a.r) * t,
...
Pixel FakeDiagnosticCore::clampPixel(Pixel pixel)
{
    return Pixel{
        clamp01(pixel.r),
        clamp01(pixel.g),
...
Pixel FakeDiagnosticCore::blur5(const Sampler& sampler, int x, int y)
{
    ...
    const TemporalContext* temporal)
{
    ...
    auto accumulate = [&](const Sampler* other) {
        if (!other) {
            return;
        }
