...
APP_NAME="Buckswood Deband v1.0"
IDENTIFIER="com.buckswood.deband.installer"
VERSION="1.0.0"
PLUGIN_BUNDLE="BuckswoodDeband.ofx.bundle"
PKGROOT="$ROOT_DIR/packaging/pkgroot"
PKG_SCRIPTS="$ROOT_DIR/packaging/scripts"
RELEASE_DIR="$ROOT_DIR/release"
DMG_STAGE="$ROOT_DIR/packaging/dmg_stage"
PKG_PATH="$RELEASE_DIR/Buckswood_Deband_v1.0_Installer.pkg"
DMG_PATH="$RELEASE_DIR/Buckswood_Deband_v1.0_Installer.dmg"
ZIP_PATH="$RELEASE_DIR/Buckswood_Deband_v1.0_macOS.zip"
...
if [[ -n "$DEVELOPER_ID_INSTALLER" ]]; then
    productsign \
        ...
        "$PKG_PATH" \
        "$RELEASE_DIR/Buckswood_Deband_v1.0_Installer_Signed.pkg"
    mv \
        "$RELEASE_DIR/Buckswood_Deband_v1.0_Installer_Signed.pkg" \
        "$PKG_PATH"
fi
...
(
    cd "$ROOT_DIR/dist"
    zip -rq "$ZIP_PATH" "$PLUGIN_BUNDLE" -x '*.DS_Store' -x '*/._*'
)
(
    cd "$RELEASE_DIR"
    shasum -a 256 \
        Buckswood_Deband_v1.0_Installer.pkg \
        Buckswood_Deband_v1.0_Installer.dmg \
        Buckswood_Deband_v1.0_macOS.zip \
        > Buckswood_Deband_v1.0_SHA256SUMS.txt
)
