...
APP_NAME="Buckswood Film Emulation v2.1"
IDENTIFIER="com.buckswood.film.emulation.installer"
VERSION="2.1.0"
PLUGIN_BUNDLE="BuckswoodFilmEmulation.ofx.bundle"
PKGROOT="$ROOT_DIR/packaging/pkgroot"
PKG_SCRIPTS="$ROOT_DIR/packaging/scripts"
RELEASE_DIR="$ROOT_DIR/release"
DMG_STAGE="$ROOT_DIR/packaging/dmg_stage"
PKG_PATH="$RELEASE_DIR/Buckswood_Film_Emulation_v2_Installer.pkg"
DMG_PATH="$RELEASE_DIR/Buckswood_Film_Emulation_v2_Installer.dmg"
...
if [[ -n "$DEVELOPER_ID_INSTALLER" ]]; then
    SIGNED_PKG_PATH="$RELEASE_DIR/Buckswood_Film_Emulation_v2_Installer_Signed.pkg"
...
cp "$ROOT_DIR/RESEARCH_NOTES.md" "$DMG_STAGE/"
cp "$ROOT_DIR/TEMPORAL_ML_BACKEND.md" "$DMG_STAGE/"
cp "$ROOT_DIR/requirements-temporal-ml.txt" "$DMG_STAGE/"
mkdir -p "$DMG_STAGE/scripts"
cp "$ROOT_DIR/scripts/temporal_ml_reconstruct.py" "$DMG_STAGE/scripts/"
cp "$ROOT_DIR/scripts/setup_temporal_ml_backend.sh" "$DMG_STAGE/scripts/"
...
(
    ...
    shasum -a 256 \
        ...
        > Buckswood_Film_Emulation_v2_Installer_SHA256SUMS.txt
)
