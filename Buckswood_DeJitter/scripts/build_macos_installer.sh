...
APP_NAME="Buckswood DeJitter v1.0"
...
PKG_PATH="$RELEASE_DIR/Buckswood_DeJitter_v1_Installer.pkg"
DMG_PATH="$RELEASE_DIR/Buckswood_DeJitter_v1_Installer.dmg"
...
if [[ -n "$DEVELOPER_ID_INSTALLER" ]]; then
    productsign \
        ...
        "$PKG_PATH" \
        "$RELEASE_DIR/Buckswood_DeJitter_v1_Installer_Signed.pkg"
    mv \
        "$RELEASE_DIR/Buckswood_DeJitter_v1_Installer_Signed.pkg" \
        "$PKG_PATH"
fi
...
(
    ...
    shasum -a 256 \
        ...
        > Buckswood_DeJitter_v1_SHA256SUMS.txt
)
